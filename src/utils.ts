    ...
    try {
      ...
      const instance: redis.RedisClient = redis.createClient({
        ...
        retry_strategy(options: redis.RetryStrategyOptions): number | Error {
          reject('[Redis] Not able to create client, reason: ' + options.error.message);
      ...
      reject('[Redis] Unknown exception during client creation, reason: ' + error.message);
    }
  });
}
export { generateId, generateError, redisFactory };
